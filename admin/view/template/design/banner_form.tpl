...
<div id="content">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-banner" class="form-horizontal">
          ...
          <div class="form-group">
            <label class="col-sm-2 control-label" for="input-status">Vị trí</label>
            <div class="col-sm-10">
              <select name="position" id="input-zone-id" class="form-control">
                <option value="category" <?php if(isset($zone_type) && $zone_type == "category"){ echo "selected"; } ?>>Danh mục sản phẩm</option>
                <option value="scroll-left" <?php if(isset($zone_type) && $zone_type == "all_page" && $zone_id == "scroll-left"){ echo "selected"; } ?>>Bên trái</option>
                <option value="scroll-right" <?php if(isset($zone_type) && $zone_type == "all_page" && $zone_id == "scroll-right"){ echo "selected"; } ?>>Bên phải</option>
                <option value="sidebar" <?php if(isset($zone_type) && $zone_type == "all_page" && $zone_id == "sidebar"){ echo "selected"; } ?>>Sidebar</option>
              </select>
            </div>
          </div>

          <?php if(isset($zone_type) && $zone_type == "category"){ ?>
          <div id="category_area" class="form-group">
              <label class="col-sm-2 control-label" for="input-status">Chọn danh mục</label>
              <div class="col-sm-10">
              <input type="text" name="path" value="<?php if(isset($category_info) && isset($category_info['name'])){ echo $category_info['name']; } ?>" placeholder="Danh mục sản phẩm" id="input-parent" class="form-control" autocomplete="off">
              <input type="hidden" name="category_id" value="<?php if(isset($category_id)){ echo $category_id; } ?>" /></div>
          </div>
          <?php } ?>
  ...
  <script type="text/javascript">
    $(function() {
      $(document).on('change', '#input-zone-id', function(){
        html = '<label class="col-sm-2 control-label" for="input-status">Chọn danh mục</label>';
        html += '<div class="col-sm-10">';
        html += '<input type="text" name="path" value="" placeholder="Danh mục sản phẩm" id="input-parent" class="form-control" autocomplete="off">';
        html += '<input type="hidden" name="category_id" value="0" /></div>';

        if($(this).val() == 'category'){
          $('#category_area').css('display', 'block');
        }else{

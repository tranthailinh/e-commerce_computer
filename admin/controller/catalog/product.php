...
class ControllerCatalogProduct extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_hotdeal'])) {
			$filter_hotdeal = $this->request->get['filter_hotdeal'];
		} else {
			$filter_hotdeal = null;
		}
		...
		if (isset($this->request->get['filter_hotdeal'])) {
			$url .= '&filter_hotdeal=' . $this->request->get['filter_hotdeal'];
		}

		if (isset($this->request->get['filter_highlight'])) {
			$url .= '&filter_highlight=' . $this->request->get['filter_highlight'];
		}
		...
		// custom configure add product link
		$data['add_gp'] = $this->url->link('catalog/product/add', 'token=' . $this->session->data['token'] . $url . '&is_gp=true', true);
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'hight_light'     => $result['is_highlight'],
				'hotdeal'     => $result['is_hotdeal'],
				'edit'       => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['token'] . '&product_id=' . $result['product_id'] . $url . $is_gp, true)
			);
		}
		...
		$data['filter_hotdeal'] = $filter_hotdeal;
		$data['filter_highlight'] = $filter_highlight;
	...
	protected function getForm() {
		...
		if(isset($this->request->get['is_gp'])){
			$data['tab_group_product'] = $this->language->get('tab_group_product');
		}
		...
		if (isset($this->request->post['hotdeal'])) {
			$data['is_hotdeal'] = $this->request->post['hotdeal'];
		} elseif (!empty($product_info)) {
			$data['is_hotdeal'] = $product_info['is_hotdeal'];
		} else {
			$data['is_hotdeal'] = 0;
		}

		if (isset($this->request->post['highlight'])) {
			$data['is_highlight'] = $this->request->post['highlight'];
		} elseif (!empty($product_info)) {
			$data['is_highlight'] = $product_info['is_highlight'];
		} else {
			$data['is_highlight'] = 0;
		}
        ...
        if (isset($this->request->get['product_id'])) {
            $data['products_gp'] = $this->model_catalog_product_configure->getConfigure($this->request->get['product_id']);
            ...
            foreach($data['products_gp'] as $key => $product_gp){
                ...
                foreach($data['products_gp'][$key]['child'] as $key1 => $value_child_product){
                    $info_child_cf_products = $this->model_catalog_product->getProduct($value_child_product['product_id']);
					$data['products_gp'][$key]['child'][$key1]['info'] = $info_child_cf_products;
                }
            }
        } else {

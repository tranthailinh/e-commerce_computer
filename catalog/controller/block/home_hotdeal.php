<?php
class ControllerBlockHomeHotdeal extends Controller {
    public function index() {
        ...
        $results = $this->model_catalog_product->getBestSellerProducts(2);
        ...
        if($results){
            foreach ($results as $result) {
                ...
                if($special){
                    $percent = '-'.(100 - round($special/$price*100)).'%';
                }else{
                    $percent = false;
                }
        ...
        return $this->load->view('block/home_hotdeal', $data);
    }
}
